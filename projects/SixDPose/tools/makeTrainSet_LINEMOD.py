"""
This file is used to generate ../data/COCO+LINEMOD_objXX dataset containing single object, 
using fusion and rendering strategy.
"""
...
window_shape = (480, 640, 3)
models_info = load_yaml(os.path.join(LINEMOD_path, 'models/models_info.yml'))

CLASSES = ['obj_{:02d}'.format(main_model_id)]    
cat2label = {cat: i + 1 for i, cat in enumerate(CLASSES)}
cat2cls = {'obj_01': 'ape', 'obj_02': 'benchvise', 'obj_04': 'cam', 'obj_05': 'can', 
        'obj_06': 'cat', 'obj_08': 'driller', 'obj_09': 'duck', 'obj_10': 'eggbox', 
        'obj_11': 'glue', 'obj_12': 'holepuncher', 'obj_13': 'iron', 'obj_14': 'lamp',
        'obj_15': 'phone'}

model_obj01_info = models_info[main_model_id]
gt_info = load_yaml(os.path.join(LINEMOD_path, 'test', '{:02d}'.format(main_model_id), 'gt.yml'))

min_x = model_obj01_info['min_x'] * scale_to_meters
min_y = model_obj01_info['min_y'] * scale_to_meters
min_z = model_obj01_info['min_z'] * scale_to_meters
size_x = model_obj01_info['size_x'] * scale_to_meters
size_y = model_obj01_info['size_y'] * scale_to_meters
size_z = model_obj01_info['size_z'] * scale_to_meters
max_x = min_x + size_x
max_y = min_y + size_y
max_z = min_z + size_z
...
BoundingBox = BoundingBox.T     # 4 x 8
...
def synthesize_by_fusion_single(image, pose=None, model_id=1):
    w, h = 640, 480
    ...
    ins_appearance_label = np.zeros(image.shape, dtype=np.uint8)
    ...
    number_of_instances = 1 #random.randint(1,3)
    ...
    for ins_idx in range(number_of_instances):
        ...
        model = LINEMOD_models['obj_{:02d}'.format(model_id)]
        if pose is not None:
            random_pose = pose
        ...
        for i in range(patience):
            ...
            for idx_dec in range(len(detection_list)):
                ...
                if (intersection / area_of_new_box > 0.3) or (intersection / area_of_existing_box > 0.3):
                    occlusion = True
                    break
...
def synthesize_by_rendering_single(pose=None, model_id=1):
    w, h = 640, 480
    ...
    for ins_idx in range(number_of_instances):
        ...
        #model = LINEMOD_models['obj_{:02d}'.format(idx_model+1)]
        model = LINEMOD_models['obj_{:02d}'.format(model_id)]
        if pose is not None:
            random_pose = pose
            closest_sampled_inplane = -1
        ...
        resize_ratio = 1.0      #np.random.uniform(0.8, 1.2)
        # patch = cv2.resize(patch, (int(patch.shape[1] * resize_ratio), int(patch.shape[0] * resize_ratio)))
        # tight_mask = tight_mask.astype(np.float32)
        # tight_mask = cv2.resize(tight_mask, (int(tight_mask.shape[1] * resize_ratio),
        #                                      int(tight_mask.shape[0] * resize_ratio)))
...
def draw_original_3d_models(image, pose=None, model_id=1):
    w, h = 640, 480
    ...
    out_mask[random_position[1]:random_position[1] + box_h, random_position[0]:random_position[0] + box_w] \
        = tight_mask
    ins_occ_agnostic_label = out_mask.astype(np.float32) * 255.0
    #out[out_mask] = col[mask]
    #out[out_mask] = image[mask] * 255
    out[out_mask] = patch[tight_mask]
    # cv2.imshow('image', out)
    # cv2.waitKey()
    seg_appearance_label[out_mask] = pixel_label[tight_mask]
    # detection: [xmin, ymin, xmax, ymax, name, pose(4x4 matrix),
    # closest_sampled_view, closest_sampled_inplane]
    detection = [random_position[0], random_position[1], random_position[0]+box_w, random_position[1]+box_h]
    #detection.append('obj_{:02d}'.format(idx_model+1))
    ...
    detection_list.append(detection)
    ins_appearance_label = ((seg_appearance_label - palette[model_id]).sum(axis=2, keepdims=True) == 0).astype(np.float32) * 255.0

    return out, seg_appearance_label, ins_appearance_label, ins_occ_agnostic_label, detection_list

def save_xml(info, out_file_path):
    node_root = Element('annotation')

    node_folder = SubElement(node_root, 'folder')
    node_folder.text = 'COCO+LINEMOD_obj{:02d}'.format(main_model_id) # modify when change model
...
def record_linemod_ann(model_meta, anno_file, data_root, img_id, ann_id, images, annotations, cls_names=[]):
    print("recording {} information for linemod dataset ...".format(', '.join(str(cn) for cn in cls_names)))
    K = cam_intrinsic

    img_type = 'real'
    if 'OCCLUSION' in anno_file:
        img_type = 'synthetic'
...
def linemod_to_coco(anno_files=[], data_roots=[], model_meta={}, cls_name=None):
    img_id = 0
    ann_id = 0
    images = []
    annotations = []
    ...
    if cls_name is None:
        ...
        categories = [{'supercategory': 'none', 'id': 1, 'name': cat2cls[CLASSES[0]]}]
    ...
    for anno_file, data_root in zip(anno_files, data_roots):
        img_id, ann_id = record_linemod_ann(model_meta, anno_file, data_root, img_id, ann_id, images, annotations, cls_names=cls_name_list)
    ...
    if cls_name is None:
        json_anno_path = os.path.join(save_path, split + '.json')
    elif isinstance(cls_name, str):
        json_anno_path = os.path.join(save_path, split + '_{}.json'.format(cat2cls[cls_name]))
    else:
        assert isinstance(cls_name, list), "cls_name should be list[str] or None !!"
        json_anno_path = os.path.join(save_path, split + '_{}.json'.format('_'.join(cat2cls[cn] for cn in cls_name)))
...
if __name__ == "__main__":
    single_fusion = True
    LINEMOD_models = {}    #dict
    print('Loading 3D models from LINEMOD dataset...')
    model_info = load_yaml(os.path.join(LINEMOD_path, 'models', 'models_info.yml'))
    
    name = 'obj_{:02d}'.format(int(main_model_id))
    LINEMOD_models[name] = Model3D()
    LINEMOD_models[name].load(os.path.join(LINEMOD_path, 'models/' + name + '.ply'), scale=scale_to_meters)
    LINEMOD_models[name].diameter = model_info[main_model_id]['diameter']
    if not os.path.exists(os.path.join(LINEMOD_path, 'models', 'fps8_{:02d}.txt'.format(int(main_model_id)))):
        sample_fps_points(model_path=os.path.join(LINEMOD_path, 'models/' + name + '.ply'), save_path=os.path.join(LINEMOD_path, 'models', 'fps8_{:02d}.txt'.format(int(main_model_id))))
    LINEMOD_models[name].fps8 = np.loadtxt(os.path.join(LINEMOD_path, 'models', 'fps8_{:02d}.txt'.format(int(main_model_id))))
    LINEMOD_models[name].fps8 *= scale_to_meters
    LINEMOD_models[name].fps8 = np.insert(LINEMOD_models[name].fps8, 3, np.ones(8, dtype=np.float), axis=1)
    LINEMOD_models[name].fps8 = LINEMOD_models[name].fps8.T
    LINEMOD_models[name].bb8 = BoundingBox
    # print(LINEMOD_models[name].fps8)

    imageset_path = os.path.join(LINEMOD_path, 'test', '{:02}'.format(main_model_id), 'rgb')

    image_name_file = open(os.path.join(LINEMOD_path, 'test', '{:02}'.format(main_model_id), 'image_names.txt'), 'r')
    ...
    for idx_image in tqdm(range(real_image_num)):
        ...
        for idx_instance in range(len(gt_info[idx_image])):
            if gt_info[idx_image][idx_instance]['obj_id'] == main_model_id:
        ...
        gt_pose = np.eye(4)
        gt_pose[0:3, 0:3] = gt_rotation.reshape((3,3))
        gt_pose[0:3, 3:4] = gt_translation.reshape((3,1))

        out, seg_app_label, ins_app_label, ins_occ_agn_label, gt_list = draw_original_3d_models(resize_img, pose=gt_pose, model_id=main_model_id)
        # cv2.imshow('1', out)
        ...
        cv2.imwrite(InstanceApp_path + '{:05d}_000.png'.format(idx_image), ins_app_label)
        cv2.imwrite(InstanceOccAgn_path + '{:05d}_000.png'.format(idx_image), ins_occ_agn_label)
    ...
    # read real training set
    with open(os.path.join(LINEMOD_path, 'training_range_obj{:02d}.txt'.format(main_model_id)), 'r') as f:
        training_indices = f.readlines()

    training_indices = [int(training_index.strip('\n')) for training_index in training_indices]
    print('the number of real training images = {:d}'.format(len(training_indices)))
    ...
    linemod_to_coco(anno_files=[os.path.join(ImageSets_path, 'val.txt')], 
                    data_roots=[save_path], model_meta=LINEMOD_models, cls_name='obj_{:02d}'.format(main_model_id))

    # synthesize training set by fusion, about 10000 images
    print('synthesize training set by fusion...')
    num_fusion_per_image = np.floor(10000 / len(training_indices)).astype(int)
    fusion_image_num = num_fusion_per_image * len(training_indices)
    print('num fusion per image = {:d}'.format(num_fusion_per_image))
    print('total number of fusion images = {:d}'.format(fusion_image_num))

    # single object fusion mode
    for idx in tqdm(range(len(training_indices))):
        idx_image = training_indices[idx]
        image = cv2.imread(image_path_list[idx_image]).astype(np.float32) / 255.0
        resize_img = cv2.resize(src=image, dsize=(640, 480))
        # cv2.imshow('1', resize_img)
        # cv2.waitKey()
        ...
        # ***********************************fusion augmentation*************************************
        for i in range(num_fusion_per_image):
            out, seg_app_label, ins_app_label_list, ins_occ_agn_label_list, gt_list = synthesize_by_fusion_single(resize_img, pose=gt_pose, model_id=main_model_id)
            # cv2.imshow('1', out)
            # cv2.imshow('label', out_label / 255.0)
            # cv2.waitKey()

            # save synthetic training images
            cv2.imwrite(JPEGImage_path + '{:05d}.jpg'.format(idx * num_fusion_per_image + i + real_image_num), out)
            cv2.imwrite(Segment_path + '{:05d}.png'.format(idx * num_fusion_per_image + i + real_image_num), seg_app_label)
            for ins_idx, (ins_app_label, ins_occ_agn_label) in enumerate(zip(ins_app_label_list, ins_occ_agn_label_list)):
                cv2.imwrite(InstanceApp_path + '{:05d}_{:03d}.png'.format(idx * num_fusion_per_image + i + real_image_num, ins_idx), ins_app_label)
                cv2.imwrite(InstanceOccAgn_path + '{:05d}_{:03d}.png'.format(idx * num_fusion_per_image + i + real_image_num, ins_idx), ins_occ_agn_label)

            # [image_name, image_height, image_width, detection_list]
            xml_gt_info = ['{:05d}.jpg'.format(idx * num_fusion_per_image + i + real_image_num), resize_img.shape[0], resize_img.shape[1], gt_list]
            save_xml(xml_gt_info, save_path + 'Annotations/' + '{:05d}.xml'.format(idx * num_fusion_per_image + i + real_image_num))


    # synthesize training set by rendering, 10000 images
    ...
    for idx in tqdm(range(rendering_image_num)):
        out, seg_app_label, ins_app_label_list, ins_occ_agn_label_list, gt_list = synthesize_by_rendering_single(model_id=main_model_id)
        # cv2.imshow('1', out)
        # cv2.imshow('label', out_label / 255.0)
        # cv2.waitKey()
    ...
    # convert to coco json format
    linemod_to_coco(anno_files=[os.path.join(ImageSets_path, 'train.txt')], 
                    data_roots=[save_path], model_meta=LINEMOD_models, cls_name='obj_{:02d}'.format(main_model_id))