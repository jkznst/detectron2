"""
This file is used to generate ../data/COCO+OCCLUSION_fusion_rendering dataset.
...
save_path = './data/COCO+OCCLUSION_fusion_rendering/'
...
# models_info = load_yaml('/data/ZHANGXIN/DATASETS/SIXD_CHALLENGE/LINEMOD/models/models_info.yml')
CLASSES = ['obj_01', 'obj_05', 'obj_06', 'obj_08', 'obj_09',
           'obj_10', 'obj_11', 'obj_12']    
...
gt_info = load_yaml(os.path.join(LINEMOD_path, 'test', '02', 'gt.yml')) # modify when change model
occlusion_model_ids = [1, 5, 6, 8, 9, 10, 11, 12]
linemod_model_ids = [1, 2, 4, 5, 6, 8, 9, 10, 11, 12, 13, 14, 15]
...
if __name__ == "__main__":
    ...
    for key, val in models_info.items():
        if key in linemod_model_ids:
            ...
            LINEMOD_models[name].diameter = val['diameter']
            if not os.path.exists(os.path.join(LINEMOD_path, 'models', 'fps8_{:02d}.txt'.format(int(key)))):
                sample_fps_points(model_path=os.path.join(LINEMOD_path, 'models/' + name + '.ply'), save_path=os.path.join(LINEMOD_path, 'models', 'fps8_{:02d}.txt'.format(int(key))))
            LINEMOD_models[name].fps8 = np.loadtxt(os.path.join(LINEMOD_path, 'models', 'fps8_{:02d}.txt'.format(int(key))))
            ...
            LINEMOD_models[name].bb8 = BoundingBox.T  # 4 x 8

    imageset_path = os.path.join(LINEMOD_path, 'test', '{:02}'.format(2), 'rgb')

    image_name_file = open(os.path.join(LINEMOD_path, 'test', '{:02}'.format(2), 'image_names.txt'), 'r')
    ...
    #     # ***********************************fusion augmentation*************************************
    #     for model_id in linemod_model_ids:
    #         if model_id in orig_model_ids:
    #             pass

    #         else:
    #             temp_gt_info = load_yaml(os.path.join(LINEMOD_path, 'test', '{:02d}'.format(model_id), 'gt.yml'))
    #             rand_idx = np.random.randint(0, len(temp_gt_info))

    #             image = cv2.imread(os.path.join(LINEMOD_path, 'test', '{:02}'.format(model_id), 'rgb',
